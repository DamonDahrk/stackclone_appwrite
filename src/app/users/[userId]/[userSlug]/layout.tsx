...
const Layout = async ({
    ...
    children: React.ReactNode;  //type of react node 
    params: { userId: string; userSlug: string }; // type of params where userId is string and userSlug is string
}) => {
    const user = await users.get<UserPrefs>(params.userId); //here we are getting the user from the server and storing it in the user variable
