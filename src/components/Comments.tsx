...
// Extend comment with authorName
export interface EnrichedComment extends Models.Document {
  content: string;
  authorId: string;
  type: "question" | "answer";
  typeId: string;
  authorName: string; // added manually by us
}
...
const Comments = ({
  ...
  // Cast initial comments into enriched type
  const [comments, setComments] = React.useState<Models.DocumentList<EnrichedComment>>({
    ..._comments,
    documents: _comments.documents.map((c: any) => ({
      ...c,
      authorName: c.authorName || "Unknown", // fallback
    })),
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ✅ Construct a proper EnrichedComment
      const enrichedComment: EnrichedComment = {
        ...
        type,
        typeId,
        authorName: user.name, // from auth store
      };

      setComments((prev) => ({
        ...prev,
        total: prev.total + 1,
        documents: [enrichedComment, ...prev.documents],
      }));
    } catch (error: any) {
        ...
        <React.Fragment key={comment.$id}>
          ...
          <div className="flex gap-2">
            <p className="text-sm">
              {comment.content} –{" "}
              <Link
                href={`/users/${comment.authorId}/${slugify(comment.authorName)}`}
