...
const QuestionCard = ({ ques }: { ques: Question }) => {
    ...
    //to attach to a div element

    React.useEffect(() => {
        if (ref.current) {   //If the ref points to a DOM element (div) then:
            setHeight(ref.current.clientHeight);  // get the height of that div and set it to state variable 'height'
        }
    }, [ref]); //runs after every render (or when 'ref' changes) 
    //React does not automatically know the rendered height of DOM elements.
    //Using ref gives you direct DOM access.
    //Using useEffect lets you measure height after render.
    //Storing height in state (height) enables other logic (like animations or styling) dependent on this height.
